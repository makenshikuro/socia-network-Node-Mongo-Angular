...
function probando(req,res) {
	res.status(200).send({message: 'Hola desde publicaciones'});
}
...
function getPublication(req, res){
	...
	Publication.findById(publicationId, (err, publication) =>{
		if(err) return res.status(500).send({message: 'Error al guardar la publicación'});
		if(!publication) return res.status(404).send({message: 'La publicación No ha sido guardada'});
...
function deletePublication(req, res){
	...
	Publication.find({user: req.user.sub, '_id': publicationId}).remove(err =>{
		...
		return res.status(200).send({message: 'Se borró la publicación'});
	});
}
	...
	if(req.files){
		...
		Publication.find({'user':req.user.sub, '_id': publicationId}).exec((err, publication) =>{
			...
			if(publication){
				...
				return removeFilesOfUploads(res,file_path, 'No tienes permisos para actualizar esta entrada');
			}
		});
